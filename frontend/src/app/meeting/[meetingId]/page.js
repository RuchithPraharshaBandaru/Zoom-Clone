...
export default function MeetingRoom(props) {
  ...
  useEffect(() => {
    if (!currentUser || !socketRef.current || !localStream) return;
  ...
  // Initialize empty local media stream on mount (0 hardware permissions, 0 camera lights, 0 mic usage on entry)
  useEffect(() => {
  ...
  const handleToggleMute = async () => {
    ...
    if (nextMuted) {
      ...
      if (webrtcRef.current) {
        webrtcRef.current.removeTrackFromPeers('audio');
      }
    } else {
      ...
      try {
        ...
        if (webrtcRef.current && track) {
          webrtcRef.current.addTrackToPeers(track, localStream);
        }
      } catch (err) {
  ...
  const handleToggleVideo = async () => {
    ...
    if (!nextVideoOn) {
      ...
      if (webrtcRef.current) {
        webrtcRef.current.removeTrackFromPeers('video');
      }
    } else {
